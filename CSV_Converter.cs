...
namespace CSV_Converter
{
    ...
    class CSVConverter
    {
        ...
        public bool Init(out string error)
        {
            ...
            string configName = "CSV_Converter.xml";
            string configPath = AppDomain.CurrentDomain.BaseDirectory;
            bool found = false;
            /* for convenience search for the configuration xml not only in the App directory.
               By moving up two directories this enables the usage of one configuration file for both Debug and Release mode.
               
             */
            for (int level = 0; level < 3; level++)
            {
                if (!File.Exists(Path.Combine(configPath, configName)))
                {
                    configPath = Directory.GetParent(configPath).Parent.FullName;
                }
                else
                {
                    found = true;
            ...
            if (!found)
            {
                error = $"Error: The configuration file CSV_Converter.xml was not found in  \"{configPath}\".";
            }
            return ReadConfig(Path.Combine(configPath, configName), out error);
        }
